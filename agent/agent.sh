...
install_agent() {
  ...
  if ! curl --fail --silent --show-error --max-time 120 "$worker_url/agent-linux-$arch" -o "$temporary"; then
    curl --fail --location --silent --show-error --max-time 120 \
      "https://github.com/imengying/CF-Monitor/releases/latest/download/agent-linux-$arch" \
      -o "$temporary"
  fi
  chmod 755 "$temporary"
  ...
  if command -v systemctl >/dev/null 2>&1; then
    printf '%s\n' \
    '[Unit]' \
    'Description=CF Monitor Rust Agent' \
  ...
  echo "CF Monitor Rust agent installed."
}
